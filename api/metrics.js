  ...
  try {
    ...
    await guard("wip", async () => {
      ...
      const byStatus = WIP_STATUSES.map((s) => ({
        status: s,
        color: WIP_COLORS[s],
        count: issues.filter((i) => (i.fields.status && i.fields.status.name) === s).length,
      }));
      const byTeam = {};
    ...
    // Blockers — flagged or "blocked" label; retry without flagged if the field rejects.
    await guard("blockers", async () => {
    ...
    // Throughput — 8 Sunday weeks; median over the 7 completed ones; current week = shipped-this-week.
    await guard("throughput", async () => {
      ...
      for (const w of weeks) {
        values.push(await countOf(`${EPIC} AND status CHANGED TO "Done" DURING ("${ymd(w.start)}", "${ymd(w.end)}")`));
      }
      out.throughput = {
